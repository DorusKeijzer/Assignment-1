...
def drawaxes(img, corners, imgpts):
    ...
    img = cv.line(img, corner, tuple(imgpts[0]), (255, 0, 0), 3)
    img = cv.line(img, corner, tuple(imgpts[1]), (0, 255, 0), 3)
    img = cv.line(img, corner, tuple(imgpts[2]), (0, 0, 255), 3)
        ...
        with np.load(f'results/Calibration_run{run}.npz') as X:
            ...
            for filename in images:
                ...
                if ret == True:
                    ...
                    imgpts, jac = cv.projectPoints(cubeaxis, rvecs, tvecs, mtx, dist)
